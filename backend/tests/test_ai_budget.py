...
def test_daily_budget_sums_estimated_cost_against_cap(db_session: Session, monkeypatch) -> None:
    settings = get_settings()
    monkeypatch.setattr(settings, "ai_daily_usd_cap", 0.45)
    db_session.execute(delete(models.AiUsage))
    db_session.flush()
