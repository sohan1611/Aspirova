"""Match local companies to bundled prestige ranks."""
...
BATCH_SIZE = 500
EXAMPLE_LIMIT = 10
PRESTIGE_PATH = Path(__file__).resolve().parents[1] / "data" / "prestige_companies.json"
...
def _load_prestige_rows(path: Path = PRESTIGE_PATH) -> list[dict[str, Any]]:
    with path.open(encoding="utf-8") as file:
        rows = json.load(file)
    if not isinstance(rows, list):
        raise ValueError("Prestige data must be a JSON array")
...
def match_prestige(
    session: Session,
    *,
    prestige_path: Path = PRESTIGE_PATH,
    dry_run: bool = False,
    reset: bool = False,
    batch_size: int = BATCH_SIZE,
) -> dict[str, Any]:
    rows = _load_prestige_rows(prestige_path)
    domain_map, name_map = build_lookup_maps(rows)

    if reset and not dry_run:
        session.execute(update(models.Company).values(prestige_rank=None))
        session.commit()
    ...
    for companies in _batches(session, batch_size=batch_size):
        for company in companies:
            ...
            if len(examples) < EXAMPLE_LIMIT:
                examples.append(f"{company.name} -> Prestige rank {rank}")
            if not dry_run:
                company.prestige_rank = rank
