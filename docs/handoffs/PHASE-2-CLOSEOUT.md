...
**Still to do:** add the same four `BACKUP_S3_*` values to GitHub Actions secrets (they're currently only in local `backend/.env`) so the real nightly cron can run unattended.
