...
- **Codex Opus = Chief Architect / CPO / Data & AI Strategist.** Designs architecture, reviews work, makes decisions, writes planning/governance docs. **Never writes implementation code, never makes PRs, never modifies app files.**
- **Codex Sonnet = Lead Engineer.** Implements against the architect's handoffs. (OpenAI Codex could replace Sonnet later; the canon, not the agent, is authoritative.)
- Switch with `/model Codex-opus-4-8` (architect) or `/model Codex-sonnet-4-6` (engineer). The user drives which is active.
...
- Commit as **`sohan1611`** (`sohanmandal1611@gmail.com`). **Never** add a `Co-Authored-By: Codex` trailer. **Never** add Dependabot. Only `sohan1611` should appear as a contributor.
