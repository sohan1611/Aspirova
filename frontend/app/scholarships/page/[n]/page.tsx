...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return landingPageMetadata(SCHOLARSHIPS_LANDING, parsePageSegment(n));
}

export default async function ScholarshipsPaginatedPage({ params }: PageProps) {
  const { n } = await params;
  const page = parsePageSegment(n);
  if (page <= 1) {
    redirect(SCHOLARSHIPS_LANDING.basePath);
  }

  const data = await loadLandingPage(SCHOLARSHIPS_LANDING, page);
